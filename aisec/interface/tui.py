...
from aisec.agents.AisecAgent import AisecAgent
from aisec.llm.config import LLMConfig
from aisec.telemetry.tracer import Tracer, set_global_tracer
...
def get_package_version() -> str:
    try:
        return pkg_version("aisec-agent")
...
class ChatTextArea(TextArea):  # type: ignore[misc]
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._app_reference: AisecTUIApp | None = None

    def set_app_reference(self, app: "AisecTUIApp") -> None:
...
class SplashScreen(Static):  # type: ignore[misc]
    ...
    def _build_welcome_text(self) -> Text:
        ...
        text.append("AISEC", style=Style(color=self.PRIMARY_GREEN, bold=True))
    ...
    def _build_tagline_text(self) -> Text:
        return Text("AI-Powered Cybersecurity Agent by CYBERSEC", style=Style(color="white", dim=True))

    def _build_start_line_text(self, phase: int) -> Text:
        emphasize = phase % 2 == 1
        base_style = Style(color="white", dim=not emphasize, bold=emphasize)
        aisec_style = Style(color=self.PRIMARY_GREEN, bold=bool(emphasize))

        text = Text("Starting ", style=base_style)
        text.append("AISEC", style=aisec_style)
...
class HelpScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("🦉 AISEC Help", id="help_title"),
            Label(
                "F1        Help\nCtrl+Q/C  Quit\nESC       Stop Agent\n"
...
class QuitScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("🦉 Quit AISEC? ", id="quit_title"),
            Grid(
...
class AisecTUIApp(App):  # type: ignore[misc]
    ...
    def _start_scan_thread(self) -> None:
        def scan_target() -> None:
            try:
                ...
                try:
                    agent = AisecAgent(self.agent_config)
    ...
    def _render_chat_content(self, msg_data: dict[str, Any]) -> str:
        ...
        if role == "user":
            from aisec.interface.tool_components.user_message_renderer import UserMessageRenderer
    ...
    def _render_tool_content_simple(self, tool_data: dict[str, Any]) -> str:
        ...
        from aisec.interface.tool_components.registry import get_tool_renderer
    ...
    def _send_user_message(self, message: str) -> None:
        ...
        try:
            from aisec.tools.agents_graph.agents_graph_actions import send_user_message_to_agent
    ...
    def action_confirm_stop_agent(self, agent_id: str) -> None:
        ...
        try:
            from aisec.tools.agents_graph.agents_graph_actions import stop_agent
...
async def run_tui(args: argparse.Namespace) -> None:
    """Run AISEC in interactive TUI mode with textual."""
    app = AisecTUIApp(args)
    await app.run_async()
