...
from aisec.interface.cli import run_cli
from aisec.interface.tui import run_tui
from aisec.interface.utils import (
...
from aisec.runtime.docker_runtime import AISEC_IMAGE
from aisec.telemetry.tracer import get_global_tracer
...
def validate_environment() -> None:  # noqa: PLR0912, PLR0915
    ...
    if not os.getenv("AISEC_LLM"):
        missing_required_vars.append("AISEC_LLM")
    ...
    if missing_required_vars:
        ...
        for var in missing_required_vars:
            if var == "AISEC_LLM":
                ...
                error_text.append("AISEC_LLM", style="bold cyan")
                error_text.append(
        ...
        error_text.append("\nExample setup:\n", style="white")
        error_text.append("export AISEC_LLM='openai/gpt-5'\n", style="dim white")
        ...
        panel = Panel(
            error_text,
            title="[bold red]🛡️  AISEC CONFIGURATION ERROR",
            title_align="center",
            border_style="red",
            padding=(1, 2),
        )
...
def check_docker_installed() -> None:
    if shutil.which("docker") is None:
        ...
        panel = Panel(
            error_text,
            title="[bold red]🛡️  AISEC STARTUP ERROR",
            title_align="center",
            border_style="red",
            padding=(1, 2),
        )
...
async def warm_up_llm() -> None:
    ...
    try:
        model_name = os.getenv("AISEC_LLM", "openai/gpt-5")
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="AISEC - AI-Powered Cybersecurity Penetration Testing Agent",
...
Examples:
  ...
  aisec --target git@github.com:user/repo.git

  # Local code analysis
  aisec --target ./my-project

  # Domain penetration test
  aisec --target example.com

  # IP address penetration test
  aisec --target 192.168.1.42

  # Multiple targets (e.g., white-box testing with source and deployed app)
  aisec --target https://github.com/user/repo --target https://example.com
  aisec --target ./my-project --target https://staging.example.com --target https://prod.example.com

  # Custom instructions
  aisec --target example.com --instruction "Focus on authentication vulnerabilities"
        """,
    )
...
def display_completion_message(args: argparse.Namespace, results_path: Path) -> None:
    ...
    panel = Panel(
        panel_content,
        title="[bold green]🛡️  AISEC CYBERSECURITY AGENT",
        title_align="center",
...
def pull_docker_image() -> None:
    ...
    if image_exists(client, AISEC_IMAGE):
        return

    console.print()
    console.print(f"[bold cyan]🐳 Pulling Docker image:[/] {AISEC_IMAGE}")
    ...
    with console.status("[bold cyan]Downloading image layers...", spinner="dots") as status:
        try:
            ...
            for line in client.api.pull(AISEC_IMAGE, stream=True, decode=True):
            ...
            error_text.append("\n\n", style="white")
            error_text.append(f"Could not download: {AISEC_IMAGE}\n", style="white")
