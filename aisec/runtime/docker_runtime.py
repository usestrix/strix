...
AISEC_IMAGE = os.getenv("AISEC_IMAGE", "ghcr.io/cybersec/aisec-sandbox:0.1.10")
...
class DockerRuntime(AbstractRuntime):
    ...
    def _create_container_with_retry(self, scan_id: str, max_retries: int = 3) -> Container:
        last_exception = None
        container_name = f"aisec-scan-{scan_id}"

        for attempt in range(max_retries):
            try:
                self._verify_image_available(AISEC_IMAGE)
    ...
    def _get_or_create_scan_container(self, scan_id: str) -> Container:  # noqa: PLR0912
        ...
        try:
            ...
            if (
                "aisec-scan-id" not in container.labels
                or container.labels["aisec-scan-id"] != scan_id
            ):
                logger.warning(
        ...
        try:
            containers = self.client.containers.list(
                all=True, filters={"label": f"aisec-scan-id={scan_id}"}
            )
            if containers:
    ...
    def _initialize_container(
        ...
        container.exec_run(
            ...
            f"AISEC_SANDBOX_MODE=true CAIDO_API_TOKEN={caido_token} CAIDO_PORT={caido_port} "
            f"poetry run python aisec/runtime/tool_server.py --token {tool_server_token} "
