...
Strix attributes its OpenRouter usage to the Strix app (via the `HTTP-Referer`,
`X-Title`, and `X-OpenRouter-Categories` headers) so it appears under Strix in
OpenRouter's [app rankings](https://openrouter.ai/apps). To attribute usage to
your own app instead, override the defaults before launching:

```bash
export OR_SITE_URL="https://your-app.com"
export OR_APP_NAME="Your App"
export OR_APP_CATEGORIES="cli-agent"  # comma-separated; set empty to omit
```
