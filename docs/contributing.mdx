...
Strix falls back to the legacy Textual scan interface for prepared scans when no compatible
...
are universal, sidecar-free packages with the same Textual fallback.
