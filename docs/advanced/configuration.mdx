...
Environment-backed keys must be updated in the launching environment and require
restarting Strix.
Timeouts, rate limits, and model-access errors do not invalidate credentials.

For Vertex AI, Bedrock, and SageMaker, Strix passively recognizes local ADC, AWS
profiles, access-key pairs, web identity, and container credential markers. It
does not contact cloud metadata services merely to detect them. A configured
cloud model still reaches the provider SDK's normal ambient credential
