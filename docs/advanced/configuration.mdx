...
<ParamField path="EXA_API_KEY" type="string">
  API key for Exa. Enables real-time web search through the Exa `/search` endpoint. This is the preferred web search provider.
</ParamField>
...
<ParamField path="STRIX_EXA_NUM_RESULTS" default="5" type="integer">
  Number of Exa results to return, from `1` to `100`. Each result includes short query-relevant highlights. This setting applies only to the Exa provider.
</ParamField>
