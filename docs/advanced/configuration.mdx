...
<ParamField path="STRIX_EXEC_FORCE_TTY" default="auto" type="string">
  Whether `exec_command` forces `tty: true` for every shell command. Defaults to on for native Windows hosts, where Docker Desktop's named-pipe transport can silently return empty output for non-tty execs. Set to `1`/`true`/`yes`/`on` to force it on any platform, or `0`/`false`/`no`/`off` to disable it (including on Windows).
</ParamField>
