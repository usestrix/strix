...
<ParamField path="--max-budget" type="number">
  ...
  Sub-agents are stopped early, at 90% of the budget, reserving the final slice
  for the root agent to wind down and produce the final report.

  As the budget is approached, graduated wrap-up warnings (at 70%, 85% and 95%)
  are surfaced to **every** agent so they can finish their work and call their
  lifecycle tool before the hard stop. Each agent is warned relative to its own
  stop point, so the bands stay reachable for both roles: the root against the
  full budget, and sub-agents against their 90% reserve (i.e. a sub-agent's 95%
  warning fires at ~85% of the full budget, just before it is stopped).
