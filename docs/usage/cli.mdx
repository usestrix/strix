...
<ParamField path="--target, -t" type="string">
  ...
  <Note>
    The mount is **writable** — the agent edits your real files, the same way a local coding agent does. Its `.git` is re-mounted read-only so your history cannot be rewritten, and system/home directories are refused as targets. Commit or stash anything you care about before scanning, and treat the mount as "scan my own code", not as isolation from untrusted code.
  </Note>
</ParamField>
