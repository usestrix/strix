...
<ParamField path="--config" type="string">
  ...
  Normal scans merge recognized process settings into this file. Explicit provider or model changes
  made in the interactive setup are saved to the selected file.
</ParamField>
...
Interactive scans use the Go/Bubble Tea interface by default. Select the
Textual interface explicitly with:

```bash
STRIX_TEXTUAL_TUI=1 strix
```

The Go interface supports targetless interactive setup for providers, credentials,
models, and targets. If the Go sidecar cannot activate, Strix can fall back to
Textual for prepared scans. Failures after Go TUI activation are surfaced rather than switching
interfaces mid-session.

Setup slash commands also cover the main fresh-run controls: `/budget <USD|off>`,
`/turns <N>`, `/scope <auto|diff|full> [base]`, `/mount <path>`,
`/target-list <path>`, and `/prompt-file <path>`. These values are applied before
the scan starts; they do not mutate an active or resumed scan.
