...
Interactive scans use the Go/Bubble Tea interface. It opens on a start screen
where a prompt, a target, or both launch a scan; a prompt on its own runs
against the working directory once you confirm mounting it. If the Go sidecar
cannot activate, Strix exits with guidance; failures after Go TUI activation are
surfaced rather than switching interfaces mid-session.

Run settings come from the flags above. The model comes from `STRIX_LLM` or your
config file, and the start screen shows which one it will use.
