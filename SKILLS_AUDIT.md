...
|---|---|---|---|---|
| 2fa-test | 2FA/MFA/OTP setup, bypass, disable flows | `strix/skills/vulnerabilities/two_factor_auth.md` | **Ported** | Ported to `strix/skills/vulnerabilities/two_factor_auth.md` (name `two-factor-auth`). Confirmed gap — prior hits were incidental 2FA mentions in csrf/bfla/race/auth0, not a flow methodology. |
...
| bac-test | Broken access control aggregator (IDOR/BOLA/BFLA/mass-assignment) | `vulnerabilities/idor.md`, `broken_function_level_authorization.md`, `mass_assignment.md` | **Merged (delta)** | Aggregator; its unguessable-ID harvesting delta merged into `idor.md` (new "Harvesting Unguessable IDs" section). BFLA/mass-assignment already covered by their own skills. |
...
| cicd-security | CI/CD pipeline attacks (Actions injection, secret exfil, runner poisoning, OIDC, supply chain) | `strix/skills/vulnerabilities/cicd_security.md` | **Ported** | Ported to Strix schema as `strix/skills/vulnerabilities/cicd_security.md`. Was a confirmed zero-coverage gap; now the first WebSkills → Strix port. |
...
| function-abuse-test | Account-function flows (change password/email, rate limits) | `strix/skills/vulnerabilities/account_security_flows.md` | **Ported** | Merged into the consolidated `account_security_flows.md` (with invite-feature-test + reset-password-test). Account-lifecycle sections ported; generic-sink sections (commenting/captcha/newsletter/review/rich-editor/social-sharing) intentionally left to their existing vuln-class skills. |
...
| idor-403-bypass | IDOR + 403/401 bypass chaining | `vulnerabilities/idor.md` | **Merged (delta)** | 403/401 bypass matrix (auth-bypass headers, URL-mutation classes, path rediscovery, tools) merged into `idor.md` as a new "403 / 401 Bypass" section. |
...
| invite-feature-test | Invitation-flow bugs (token leak, IDOR, role escalation) | `strix/skills/vulnerabilities/account_security_flows.md` | **Ported** | Merged into the consolidated `account_security_flows.md` (Invitation & Membership section: token leak/non-expiry, email IDOR, ghost membership, role race/Ghost Admin, U+3164 Hangul filler, second-admin 2FA disable). |
...
| reset-password-test | Password-reset ATO (host-header poisoning, IDN, token issues) | `strix/skills/vulnerabilities/account_security_flows.md` | **Ported** | Merged into the consolidated `account_security_flows.md` (Password Reset section: host-header poisoning, email-param manipulation, weak/non-invalidated tokens, response manipulation, IDOR-to-ATO, Referer leak). Cross-references `header_injection` for header mechanics. |
...
| session-fixation-test | Session fixation methodology | `strix/skills/vulnerabilities/session_fixation.md` | **Ported** | Ported to `strix/skills/vulnerabilities/session_fixation.md`. Confirmed gap — session fixation was only name-dropped in authentication_jwt/oauth, no methodology. Cross-references crlf/header_injection/oauth/two-factor-auth for chained delivery. |
