...
Supports OpenAPI 3.x, Swagger 2.0, and Postman Collection v2.1. Two things must
happen on the host, in code, and nothing more:

* **Detection** — deciding that ``--target ./openapi.yaml`` is a contract and not
  an unrelated JSON/YAML file (:func:`detect_spec_format`).

* **Base URLs** — scope authorization has to be deterministic and cannot be
  self-granted by the agent, so the in-scope hosts a spec declares are read here
  (:func:`spec_base_urls`).

Everything else about the contract — operations, parameters, request bodies,
auth schemes — is left to the agent, which reads the spec file directly in the
sandbox. That resolves ``$ref``, ``allOf``, and nested schemas properly instead
of re-implementing a spec parser here, and keeps the full document available
rather than a summarized excerpt.

Collections held only in Postman are fetched here too, since the API key stays
on the host and never enters the sandbox.
"""
