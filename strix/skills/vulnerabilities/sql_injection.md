...
0. For a boolean/time-based oracle, load the `deterministic_verification`
   skill (`custom` category) alongside this one and run its paired-request
   diff, requiring it to reproduce across repeat runs before setting
   `confidence: "high"` — a single "the response
