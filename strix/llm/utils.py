...
def strip_invalid_api_tags(content: str) -> str:
    """Strip internal reasoning/analysis tags that are not valid in OpenAI-compatible APIs.

    This function removes tags like <channel>, <analysis>, <final>, etc. that may be
    generated by LLMs with reasoning capabilities but are not compatible with
    OpenAI-compatible API implementations. Tool invocations are preserved.

    Args:
        content: The content to clean

    Returns:
        Content with invalid API tags removed
    """
    if not content:
        return ""

    # Only remove the problematic internal reasoning/analysis tags that break OpenAI APIs
    # These patterns preserve tool invocations like <function=...>, <invoke>, etc.
    invalid_api_tags = [
        r"<analysis>.*?</analysis>",
        r"<final>.*?</final>",
        r"<channel>.*?</channel>",
    ...
    cleaned = content
    for pattern in invalid_api_tags:
