...
@pytest.mark.asyncio
async def test_interactive_nudge_offers_waiting_on_what_was_said() -> None:
    """The nudge is what strands an agent that answered in plain text.

    It has to say that the text already landed, or the agent restates it just to
    reach a tool call and the user reads the same answer twice.
    """
...
@pytest.mark.asyncio
async def test_interactive_nudge_does_not_claim_silence_was_delivered() -> None:
    """A turn can end with no tool call and no text at all.

    Telling that agent its words already landed would invite it to wait on an
    answer the user never received, leaving them in front of a bare prompt.
    """
    items = await execution._append_tool_required_message(
        session=None,
        context={"parent_id": None},
    ...
    nudge = items[0]["content"]
    assert "already been delivered" not in nudge
    assert "respond_to_user with no message" not in nudge
    assert "read nothing" in nudge


def test_turn_produced_text_reads_what_the_turn_actually_said() -> None:
    for final_output, spoke in (
        ...
        assert execution._turn_produced_text(result) is spoke, final_output
